...
> Submitted by: DQF orchestrator (2026-07-31) — evidence back-fill task (P4 Task 1)
> Basis: 80 live queries (10 drugs × 8) against `balade-pubmed-rag-bot.hf.space/search?q=...&k=3`, 240 snippets adjudicated by 2 analyst lanes; plus prior pipeline history (TICKET-001/002/003).

## 1. Critical: response text is truncated (~512 chars) — this is the #1 blocker

- The endpoint's `text` field (title + abstract) is capped at ~512 characters, cutting abstracts mid-sentence. Verified: snippet length 513 chars, PMID:40032809 truncated after "...for children w". Analyst lanes independently confirmed systematic truncation across all 96 snippets sampled.
- **Impact**: no quantitative value (NNT, BP reduction, A1c change, onset) survives intact in any back-fill snippet → L4 adjudication could not update a single value, even where drug-relevant RCTs were returned (e.g. terazosin PMID:2872809 truncated at "After a two-week placebo lead-in period, e").
...
- Raw results duplicated across `l3_output/`, legacy `raw/`, `l3_output_backup_pre_eutils/`; digests live in `%TEMP%` (volatile); 87 profiles exist for 88 drugs (`aluminum-magnesium-hydroxide` has no profile); 26 drugs have no L2b digest.
...
- 80/80 RAG queries succeeded (0 still-loading on a warm endpoint), 240 snippets saved full-length to `rag-queries/l3_output/<drug>/backfill_<slug>.json` (new files only, no overwrites) + `rag-queries/backfill_queries.py` (resumable runner).
- Adjudication result: **0 L4 value changes** — current class-anchored values retained for all 10 drugs (alogliptin, trandolapril, methyldopa, exenatide, rosiglitazone, pramlintide, terazosin, saxagliptin, furosemide, vildagliptin) as "insufficient evidence", not "confirmed". The truncated-abstract blocker (item 1) is the single fix that would unlock a real back-fill.
